...
bool CYOA::startGame() {
    ...
    while(current_input != 'q') {
        ...
        switch(current_input) {
            ...
            // if it's a cycle back to the same room we don't need to change much
...
bool CYOA::parseInputFile(std::string fn) {
    ...
    std::string * names = new std::string[numLines];
    int names_count = 0;
    ...
    for(int i = 0; i < numLines; i++) {
        ...
        int j = 1;
        for(; line[j] == ' '; j++); // ignore any leading whitespace on the argument after the command character
    ...
    if(names_count != graph.getNumVertices()) {
        std::cout << "Error : Not All Tag Names Are Defined\n";
        return false;
    }
